...
static const QString outputPath = "msaa.png";
...
int main(int argc, char** argv)
{
    ...
    if (!rhi->isFeatureSupported(QRhi::Feature::MultisampleTexture)) { return EXIT_FAILURE; }
    ...
    {
        msaaTexture.reset(
            rhi->newTexture(QRhiTexture::RGBA8, QSize(1280, 720), 8,
    ...
    cmdBuffer->beginPass(renderTarget.get(), clearColor, dsClearValue, resourceUpdates);
    {
        ...
        cmdBuffer->setViewport(QRhiViewport(0, 0, msaaTexture->pixelSize().width(),
                                            msaaTexture->pixelSize().height()));
        ...
        QRhiReadbackDescription rb(msaaResolveTexture.get());
